      ...
      <aside className="w-16 lg:w-64 bg-panel-deep border-r border-divider flex flex-col fixed h-full transition-all duration-300">
        <div className="h-16 border-b border-divider flex items-center px-4 lg:px-6">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-display text-xl font-bold text-text-primary hidden lg:block">AI 漫剧</span>
          </Link>
        </div>
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          <Link href="/projects" className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all", pathname === "/projects" || pathname?.startsWith("/projects/") ? "bg-anime-purple/10 text-anime-purple font-medium border border-anime-purple/20" : "text-text-secondary hover:bg-panel-mid hover:text-text-primary")}>
        ...
        <div className="p-3 lg:p-4 border-t border-divider space-y-3">
          <Link href="/settings/models" className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors", pathname?.startsWith("/settings") ? "bg-anime-purple/10 text-anime-purple font-medium" : "text-text-secondary hover:bg-panel-mid hover:text-text-primary")}>
            <KeyIcon className="w-5 h-5 flex-shrink-0" />
            <span className="hidden lg:block">模型中心</span>
          </Link>
