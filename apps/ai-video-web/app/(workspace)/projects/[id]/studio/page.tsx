    ...
    <div className="h-screen flex flex-col bg-cinema">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-auto">
          ...
          {activeTab==="story"&&(<div className="p-6 max-w-3xl"><h2 className="font-display text-2xl font-bold text-white mb-6">故事编排</h2><Textarea placeholder="输入你的故事或剧情描述..." value={prompt} onChange={e=>setPrompt(e.target.value)} className="min-h-[200px] mb-4"/><div className="mb-4"><ShotCharacterBinding projectId={projectId} shot={{ id: 'story', sequence: 0, prompt: '' } as any} selectedCharacterIds={selectedCharacterIds} onChange={setSelectedCharacterIds} /></div><Button onClick={handleGenerateStoryboard} isLoading={generateMutation.isPending} disabled={!prompt.trim()} className="gap-2"><SparklesIcon className="w-4 h-4"/>{generateMutation.isPending?"生成中...":"生成分镜"}</Button></div>)}
